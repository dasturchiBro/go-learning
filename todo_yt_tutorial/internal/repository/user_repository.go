...
func CreateUser(pool *pgxpool.Pool, user *models.User) (*models.User, error) {
	...
	err := pool.QueryRow(ctx, query, user.Email, user.Password).Scan(&user.ID, &user.Email, &user.CreatedAt, &user.UpdatedAt)
...
func GetUserByID(pool *pgxpool.Pool, id string) (*models.User, error) {
	...
	var user models.User
	err := pool.QueryRow(ctx, query, id).Scan(&user.ID, &user.Email, &user.CreatedAt, &user.UpdatedAt)
	if err != nil {
		return nil, err
	}
	return &user, nil
